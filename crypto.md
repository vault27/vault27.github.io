...
```
A PRF is used in cryptographic protocols to transform shared secrets and contextual data into deterministic, unpredictable,and independent cryptographic keys required for secure communication
Used in all secure protocols
```
- A function is a rule that maps input to exactly one output
- Deterministic = same input → always same output, No randomness, no variation
- PRF = Pseudo-Random Function
- PRF = function that produces pseudorandom output from key + input
- PRF = deterministic way to turn secret + context → secure key material
- `shared secret → PRF → clean, random-looking keys`
- Any secure keyed construction that is indistinguishable from random can be used as a PRF
- Examples of PRF: HMAC-based (HMAC-SHA256, HMAC-SHA1, HMAC-SHA512), CMAC (block cipher based), KMAC (modern, SHA-3 based)
- It is called "pseudo-random" because the output only looks random,but is actually deterministic and generated by a fixed algorithm

**Why PRF is used (fundamental reasons)**

- Turn shared secret into usable keys
- Raw secrets (like DH g^xy) are not suitable directly
- Ensure both sides derive identical keys
- Make output look random (to attacker)
- Bind context into keys: secret + nonces + identities + SPI + protocol data - prevents: replay, key reuse, cross-protocol attacks
- Key separation (very important) - From one secret, derive many keys: encryption key, integrity key, authentication key

**Sequence**

1. Takes a secret key
2. Takes some input data
3. Produces "random-looking" output
4. Same input → same output
5. Without key → output is unpredictable

**Core properties of a PRF**

- Keyed function - Has a secret key k
- Deterministic: Same key + same input → same result
- Pseudorandomness (main property) - Output is indistinguishable from random - Without knowing key → attacker cannot predict outputs
- Unpredictability
- Input sensitivity (avalanche effect) - small change in input → completely different output

**PRF vs HKDF vs KDF**

```
PRF  = primitive (single keyed function)
KDF  = concept (any method to derive keys)
HKDF = specific KDF (built from PRF/HMAC)

PRF → used to build → KDF
KDF → can be implemented as → HKDF
```

**KDF usually uses PRF:**

```
K1 = PRF(secret, data1)
K2 = PRF(secret, K1 | data2)
```

**KDF Examples**

- IKEv1 SKEYID expansion
- IKEv2 PRF+
- TLS key schedule
...
```
HKDF = specific, standardized KDF
Defined in: RFC 5869
```
...
```
Most protocols use HMAC as PRF
And different kinds of KDF
```

**How PRF prevents key reuse attacks**
