...
- SPI is a field or fields in a packet header of IKE, ESP, or AH protocols
- Why is it needed? `To demultiplex IKE/ESP/AH traffic.` It is like port numbers in TCP/UDP. Based on SPI node understand which SA to use to process particular traffic flow

`SPI is used in two contexts:` 

- IKE SPI
- `IPSec SPI` - generated and exchanged in Phase 2 in IKEv1 or in IKEv2 during CHILD SA creation (IKE_AUTH or CREATE_CHILD_SA)

SPIs are also described using different perspectives: `local and remote, inbound and outbound, initiator and responder`:

- Outbound/Inbound
  - Outbound and inbound logic is relevant only for Phase2/IKE_AUTH SPIs and used in: AH, ESP
  - `Outbound SPI` - The SPI my router inserts into ESP/AH  I send
  - `Inbound SPI` - The SPI my router expects to see in ESP/AH packets I receive
- Initiator/Responder
  - Used in IKE packets
  - `Inititor SPI` - generated by node, which started IKE conversation
  - `Responder SPI` - generated by node, which accepted IKE connection
- Local/Remote
  - These terms are used only in IKE SA description inside the node logic
  - `Local SPI`  = the SPI I generated
  - `Remote SPI` = the SPI peer generated

**IKE SPI**

- IKE SPI `generated by each node and transfered in Phase 1 IKEv or IKE_SA_INIT in IKEv2,`
- `Used in all IKE packets`
- Every IKE packet `contains 2 SPIs` 64 bits each:

```
Initiator SPI:  a1b2c3d4e5f60708
Responder SPI:  a1b2c3d4e5f4a1bb
```

- IKE SA is uniquely identified by these `2 SPIs`
...
- IPSec SPI is generated by both peers and exchanged during Phase 2 IKEv1 or IKE_AUTH INKEv2
- Each node generates IPSec SPI and sends it to a peer, it means: use this number when you send ESP/AH packets to me
- This number is injected into the header of  every ESP/AH, so the remote peer knows which Security Assosiation (SA) `[cryptographic parametres and keys]` to use for decryption and authentication
- `Only one SPI field in every ESP/AH packet, not two, as in IKE SPI`
- IPSec tunnel SA is uniquely identified by `1 SPI`, but at the same time there are `2 SAs` in routers memory for each IPSec tunnel: `Inbound SA and Outbound SA`
- Inbound SA is identified by inbound SPI - this SPI was received from the peer
- Outbound SA is identified by outbound SPI - this SPI was generated locally
- For every new IPSec tunnel - because of different traffic selectors - new pairs of SPI are generated by both sides

**IPSec SPI Flow**

![IPSec SPI Flow](/images/ipsec/ipsec_spi.svg)
...
- SAs are used in both meanings in both tunnels: Control and Data
- Control and Data tunnels  have different SAs
...
- The `set of transforms` inside one proposal defines the full Phase 1/IKE_INIT policy - `transform set`
...
- Inbound SA for IPSec: how to decrypt and authenticate packets arriving at the device based on SPI inside ESP protocol
- Outbound SA for IPSec: How to encrypt and authenticate packets leaving the device based on SPI inside ESP protocol
- `Both SAs (inbound and outbound) use the same crypto parameters, negotiated during Phase 2/IKEv1 or IKE_INIT/IKEv2`
...
```
Initiator → Responder : SA, Nonce, traffic selectors, SPI, key exchange - optional, protocol - ESP/AH,
...
- Provides Confidentiality, authentication, replay protection
...
|   (e.g., TCP, UDP)     |
+----------------------------+
| ESP Trailer              |  ← Encrypted + Authenticated
|   (Padding, Pad Length,  |
|    Next Header)          |
+----------------------------+
...
|    Next Header)          |
+----------------------------+
| ESP Auth Trailer (ICV)   |  ← Not encrypted; Integrity Check Value (HMAC)
+----------------------------+
```
...
- Looks up SAD based on SPI to find:
    - Encryption key
    - Integrity key
    - Mode, lifetime, etc.
...
- `It authenticates the packets, not the peers`
- Peers are authenticated via IKE
- Does not support encryption
- AH is incompatible with NAT because it authenticates immutable IP header fields
- Transform set for authentication header defines only HMAC function, for example `ah-sha-hmac`
- Not used anywhere
- AH has two modes just like ESP: Transport mode and Tunnel mode
- Transport mode is used for `Host-to-host security`
- Tunnel mode is used for `Gateway-to-gateway VPN - Secure communication between networks`

**AH – Transport Mode**

- Protects host-to-host traffic without encapsulation
- Transport mode calculates HMAC based on `Immutable IP header fields, AH header, Entire IP payload` and shared secret symmetric key derived during IKE, then  puts result to field `Integrity Check Value (ICV)`
- Supported HMAC algorithms: `HMAC-MD5-96, HMAC-SHA1-96, HMAC-SHA2-256-128, HMAC-SHA2-384-192, HMAC-SHA2-512-256`
- Realistic Use Case (Historical): `Secure Router-to-Router Routing Protocols, Secure Server-to-Server Communication`
...
- Protects `entire original IP packet` with encapsulation
- Tunnel mode calculates HMAC based on `Immutable or predictable fields of the OUTER IP header + AH header (with ICV field set to zero) + Entire INNER IP packet` and shared secret symmetric key derived during IKE, then  puts result to field `Integrity Check Value (ICV)`
- IP 51 is used for new IP packet
- Supported HMAC algorithms: `HMAC-MD5-96, HMAC-SHA1-96, HMAC-SHA2-256-128, HMAC-SHA2-384-192, HMAC-SHA2-512-256`
- Realistic Use Case (Historical): `Government / Military Networks
...
- Original purpose: tunnel PPP over IP networks for ISPs
- Today: mainly VPNs (L2TP over IPsec)
- Advanced: L2TPv3 for generic Layer 2 tunneling in carrier networks
- Originally, L2TP evolved from L2F (Cisco) and PPTP (Microsoft) to allow ISPs to tunnel PPP sessions from customers to a central server
- Example: A DSL provider could send a customer’s PPP session over the Internet to a BRAS (Broadband Remote Access Server)
- This allows authentication, accounting, and IP assignment at the ISP, even though the customer is on a remote IP network
- Used for client VPNs
- Some carriers use L2TP (or L2TPv3) to tunnel Layer 2 frames across IP/MPLS networks
- `L2TPv3 can encapsulate Ethernet, Frame Relay, ATM, not just PPP`
...
- We cannot use PPP above UDP directly because:
  - L2TP adds a tunnel ID and session ID
  - L2TP allows multiple PPP sessions over a single UDP/IP tunnel
  - L2TP defines control messages (like starting/ending sessions, managing tunnels). PPP itself has no concept of tunneling over IP
  - Standardization & Interoperability: Using L2TP over UDP/IP became a standard for VPNs, so vendors could interoperate reliably
...
**Before decryption, the packet on the wire would look like:**

```
Ethernet
Outer IP (IPsec peers)
ESP
Encrypted Payload
```

**After Wireshark decrypts ESP, it reveals:**
...
Internet Protocol Version 4
    ...
    Differentiated Services Field: 0x00
    Total Length: 1400
    Identification: 0x5c11
    Flags: 0x4000
    Fragment Offset: 0
    Time to Live: 64
    Protocol: TCP (6)
    Header Checksum: 0xa811
...
Transmission Control Protocol
    ...
    Sequence Number: 148221
    Acknowledgment Number: 892211
...
- IPsec alone cannot natively send multicast to multiple endpoints without setting up separate Security Associations (SAs) for each receiver
- When you combine GRE + IPsec, the stack is:

```
[IPsec ESP Header]
    └─ [GRE Header]
         └─ [Original Multicast IP Packet]
```

- Now, IPsec sees only unicast GRE packets between the two tunnel endpoints
- GRE handles encapsulating the multicast traffic into unicast tunnels
- For two routers talking OSPF over IPsec, GRE isn’t strictly necessary
- GRE is mostly needed when you want one tunnel to carry multicast traffic to multiple endpoints efficiently
- OSPF can run directly over IPsec, but it’s practical only for simple point-to-point site-to-site tunnels

## 10 IPSec tunnels and MTU

- MTU (Maximum Transmission Unit) is the largest packet size a network interface can transmit
- Standard Ethernet MTU = 1500 bytes.
- IPsec adds headers:
  - ESP header (20–32 bytes)
  - ESP trailer + padding (variable, up to 256 bytes depending on block size)
  - AH header (if used)
- Result: effective MTU for payload is smaller
- If you ignore this:
- Packets larger than the path MTU may be dropped
- TCP may fragment or experience black hole issues if DF (Don’t Fragment) is set
- VPN throughput suffers, intermittent connectivity, strange retransmissions
...
- IPsec relies on PMTUD to detect the maximum packet size that can traverse the path
- Problem: PMTUD often fails with firewalls/NAT that block ICMP “fragmentation needed” messages
- Result: large packets are dropped silently → “black hole” problem

**Best Practices for MTU in IPsec**

- Reduce MTU on tunnel interface ≈ 1400–1420
- ip tcp adjust-mss ensures TCP sessions automatically reduce segment size, preventing fragmentation
- If NAT traversal is used, add UDP/4500 encapsulation overhead (~28 bytes)
- Use ping with DF set to test path MTU: `ping 10.2.2.1 df-bit size 1400`
- Gradually adjust MTU until it succeeds without fragmentation
- Use Wireshark to verify ESP packets and sizes
- Document MTU and MSS in your network design
- Force all TCP sessions going through the VPN to use a smaller MSS, so they never exceed the IPsec MTU - TCP MSS clamping
- Done on firewall or VPN gateway
- Example:

```
interface outside
 ip access-group VPN_MSS in
!
ip access-list extended VPN_MSS
 permit tcp any any mss 1360
```
What this does:

- When a TCP SYN packet enters the firewall/VPN, the gateway modifies the MSS field in the TCP header if it’s larger than 1360
