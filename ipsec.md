...
- `SPI is used in two contexts:` 
  - `IKE SPI`- negotiated in Phase 1 IKEv1 or IKE_SA_INIT IKEv2, used in IKEv1 and IKEv2
  - `IPSec SPI` - negotiated in Phase 2 in IKEv1 or in IKEv2 during CHILD SA creation (IKE_AUTH or CREATE_CHILD_SA)
- SPIs are described using different perspectives: `local and remote, inbound and outbound, initiator and responder`
- Outbound/Inbound
...
- Initiator/Responder
  - `Inititor SPI` - used only in IKE - generated by node, which started IKE conversation
  - `Responder SPI` - used only in IKE - generated by node, which accepted IKE connection


**IKE SPI**

- IKE SPI `generated and transfered in Phase 1 IKEv or IKE_SA_INIT in IKEv2,`
- `Used in all IKE packets`
- Every IKE packet `contains 2 SPIs` 64 bits each, for example in first packet from initiator:
